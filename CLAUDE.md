...
- **Test fixtures are generated, not committed by hand.** `npm run fixtures:pades` rebuilds both
  the M0 signed PDF and the parser fixtures in `src/lib/fixtures/` (see the README there for what
  each one is for). Fresh keys every run, so never assert on fixture bytes. Never commit a real
  signed document or a `.p12`.
